...
r"""Servidor WebSocket para o agente Leonidas (AI Studio e Web UI local).

Este script inicia um servidor WebSocket que alimenta o agente Leonidas.
O agente recebe áudio e vídeo, processa-os usando GenAI Processors e
usa a API Gemini Live para gerar respostas colaborativas em tempo real.

Este servidor pode ser usado com duas interfaces diferentes:
1. Um applet no Google AI Studio (recomendado).
2. Uma aplicação web local servida a partir do diretório `ais_app`.

Veja `leonidas.py` para a lógica principal do agente.
...
2. **Definir Variável de Ambiente**:
   Você deve definir sua chave da API do Google como uma variável de ambiente.
   No Windows:
   ```shell
...
Após a configuração, inicie o servidor WebSocket executando este script:
```shell
python examples/live/leonidas_ais.py
```

Em seguida, conecte-se a ele usando uma das opções de frontend descritas no
arquivo `examples/live/README.md`.
"""
...
class LeonidasConfig:
  """Config for the Leonidas agent."""

  chattiness: float = 0.5

  @classmethod
  def from_json(cls, json_config: str) -> 'LeonidasConfig':
    """Creates a LeonidasConfig from a JSON config."""
    json_dict = json.loads(json_config)
    config = LeonidasConfig()
...
class AIStudioConnection:
  ...
  def __init__(self, ais_ws: ServerConnection):
    ...
    self.leonidas_config = LeonidasConfig()
  ...
  async def receive(self) -> AsyncIterable[content_api.ProcessorPart]:
    ...
    async for json_part in self._ais_ws:
      ...
      if part.is_image() or part.is_audio():
        ...
        self.leonidas_config = LeonidasConfig.from_json(
            part.base64data
        )
        logging.debug(
            '%s - Config received: %s',
            time.perf_counter(),
            self.leonidas_config,
        )
        self.is_resetting = True
        return
      else:
...
async def live_leonidas(ais_websocket: ServerConnection):
  """Runs the Leonidas agent on AI Studio input/output streams."""
  ais = AIStudioConnection(ais_websocket)

  # Running in a loop as the agent can receive a RESET command from AIS, in
  # which case the live agent loop needs to be reinitialized.
  while True:
    leonidas_processor = leonidas.create_leonidas_agent(
        API_KEY,
        chattiness=ais.leonidas_config.chattiness,
        unsafe_string_list=None,
    )
    try:
      await ais.send(leonidas_processor(ais.receive()))
    except Exception as e:  # pylint: disable=broad-exception-caught
      logging.debug(
          '%s - Resetting live agent after receiving error : %s',
          time.perf_counter(),
          e,
      )
