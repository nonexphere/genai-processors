...
MODEL_LIVE = 'gemini-2.5-flash-live-preview'

# Model to use for the event detection processor.
MODEL_DETECTION = 'gemini-2.5-flash-lite-preview-06-17'
...
COMMENT_MSG = (
    'Continue a análise do que você vê, sem repetir os mesmos comentários.'
    ' Lembre-se que esta é uma colaboração para atingir um objetivo. Não cancele'
    ' a tarefa a menos que o usuário peça explicitamente.'
)
# Resposta quando o agente detecta algo especial que deve interromper.
INTERRUPT_COMMENT_MSG = (
    'Interrompa o comentário atual para focar no que você vê agora. Se não'
    ' houver nenhum comentário em andamento, inicie um novo. Comece seu'
    ' comentário como se estivesse interrompendo a si mesmo ou um silêncio.'
    ' Comente sobre o novo evento que acabou de acionar esta interrupção. Se'
    ' este evento estiver relacionado ao que você pediu ao usuário para fazer,'
    ' continue o comentário ou as instruções anteriores. Não cancele a tarefa.'
)

# Resposta quando o agente esperou demais pela resposta do usuário.
WAIT_FOR_USER_MSG = (
    'Retome a análise. Não repita os mesmos comentários, mas verifique se algo'
    ' explica por que o usuário não está respondendo.'
)

# Resposta quando o agente está esperando por um sinal do usuário e recebe uma interrupção.
INTERRUPT_WAIT_FOR_USER_MSG = (
    'Verifique se você precisa fazer uma chamada `wait_for_user` para esperar'
    ' que algo aconteça. Se fizer uma chamada `wait_for_user`, fique em'
    ' silêncio, não produza nada além dessa chamada. Caso contrário, comece a'
    ' comentar, ou responda à solicitação do usuário ou passe para as próximas'
    ' etapas de suas instruções ou da conversa.'
)

# Resposta quando o agente diz algo errado ou inseguro.
START_AGAIN_MSG = (
    'Ocorreu uma falha, não consegui receber sua última mensagem, o que você'
    ' estava dizendo?'
)
...
LEONIDAS_PROMPT_PARTS = [
    ...
    (
        'Você é Leonidas, uma IA colaborativa para arquitetar, especificar e'
        ' construir software complexo junto com seu desenvolvedor, Guilherme.'
    ),
    (
        'Seu objetivo é ser um parceiro proativo, usando sua percepção do'
        ' ambiente (câmera, tela, áudio) para auxiliar nas tarefas.'
    ),
    # Tom e Linguagem
    (
        'Comunique-se em português do Brasil. Seu tom é profissional, analítico'
        ' e colaborativo. Seja claro, conciso e estruturado.'
    ),
    # Princípios Operacionais
    (
        'Pense com base em primeiros princípios. Desconstrua problemas de forma'
        ' rigorosa. Se uma solicitação for ambígua, peça esclarecimentos'
        ' específicos antes de prosseguir.'
    ),
    (
        'Seja proativo ao explicar conceitos técnicos ou decisões de design,'
        ' garantindo o entendimento mútuo.'
    ),
    # Interatividade
    (
        'Você pode e deve ser interrompido. Se eu começar a falar, pare o que'
        ' estiver dizendo e ouça. Responda às minhas perguntas diretamente'
        ' antes de continuar.'
    ),
    (
        'Use as ferramentas disponíveis para buscar informações ou executar'
        ' tarefas quando julgar útil para o nosso objetivo.'
    ),
]
...
EVENT_DETECTION_PROMPT = (
    'Você é um agente que detecta eventos no feed de vídeo. Você recebe'
    ' imagens de uma câmera. Você deve detectar sempre que uma pessoa ou um'
    ' grupo de pessoas estiver de frente para a câmera e perto o suficiente'
    ' para iniciar uma conversa. Você também deve detectar sempre que a câmera'
    ' estiver apontando para uma tela de computador.\n'
    'Sempre que vir uma pessoa ou um grupo de pessoas perto o suficiente da'
    ' câmera,'
    f" responda com '{EventTypes.DETECTION}'.\n"
    'Sempre que vir uma tela de computador, responda'
    f" '{EventTypes.DETECTION}'.\n"
    'Sempre que vir algo especial, o usuário querendo te parar, interagir com'
    ' você ou sempre que vir algo novo que mudaria seu comentário, responda com'
    f" '{EventTypes.INTERRUPTION}'. Não interrompa se o usuário estiver falando"
    ' com você, apenas quando algo relacionado ao feed de vídeo mudar'
    ' significativamente.\n'
    f"Em todos os outros casos, responda com '{EventTypes.NO_DETECTION}'.\n"
)
...
class LeonidasAgent(processor.Processor):
  """Processor for the Leonidas agent, generating real-time, contextual collaboration.

  The audio and video parts to analyze should have the `realtime`
  ...
  def __init__(
    ...
    Args:
      ...
      chattiness: Probability of triggering a proactive comment when the model
        has finished talking or every 3 seconds. Set to 0 to disable proactive
        commenting.
  ...
  def _start_commentating(
    ...
    if self._commentator.id is None:
      logging.debug(
          '%s - No agent id, ignoring start_commentating: %s',
          time.perf_counter(),
          self._commentator,
      )
    else:
...
def create_leonidas_agent(
  ...
  r"""Creates the Leonidas agent.

  The Leonidas agent processor takes audio and video as input and produces
  real-time, contextual, collaborative audio responses. The agent's speech is
  interrupted by events detected on the video stream or by user speech. Input
  audio and video streams coming from devices should be passed to the processor
  with the `realtime` substream name.

  Args:
    api_key: The API key to use for the model.
    chattiness: Probability of triggering a proactive comment when the model has
      finished talking or every NO_COMMENT_DELAY_SEC seconds. Set to 0 to
      disable proactive commenting.
    unsafe_string_list: a list of strings that should not be sent back to the
      ...
      When set, the agent will interrupt itself if the model outputs this
      string and will not output the rest of the response.

  Returns:
    A Leonidas agent processor.
  """
